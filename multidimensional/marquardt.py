...
from visualization import plot_descent, plot_surface_descent
import time


def marquardt(f, grad, hessian, x0, eps, M, mu=1e4):
    n = len(x0)
    history = [x0.copy()]
    xk = x0.copy()
    x_star = xk
    ...
    for k in range(M+1):
        ...
        if np.linalg.norm(x_next - xk) < eps and abs(f_next - fk) < eps:
            xk = x_next
            history.append(xk)
...
if __name__ == '__main__':
    ...
    x_star, k, history = marquardt(f, grad, hessian, x0, eps, M)
    ...
    # plot_surface_descent(f,history, x_star)
